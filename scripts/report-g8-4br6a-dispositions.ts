...
import { buildG8V2DispositionAggregateReport, G8_V2_DISPOSITION_DEFAULT_PATHS, loadG8V2DispositionPlan, type G8V2DispositionPaths, type G8V2DispositionPlan } from './lib/g8V2ConflictDisposition.js';
...
for (let index = 0; index < process.argv.slice(2).length; index += 1) {
  ...
  if (!valueFlags.has(flag)) throw new Error(`unsupported BR6A disposition argument: ${flag}`);
...
if (existsSync(planOutput)) throw new Error('BR6A plan output already exists; refusing overwrite');
...
let outputPlan: G8V2DispositionPlan | null = loadG8V2DispositionPlan(paths, certifiedPlan);
const reportText = `${JSON.stringify(buildG8V2DispositionAggregateReport(outputPlan), null, 2)}\n`;
...
if (verifyReplay) {
  ...
  if (replayOutput === planOutput || existsSync(replayOutput)) throw new Error('BR6A verify-replay output collision');
  const childArguments = process.argv.slice(2).filter((argument) => argument !== '--verify-replay');
  const outputIndex = childArguments.indexOf('--plan-out');
  if (outputIndex < 0) throw new Error('BR6A verify-replay output argument missing');
  ...
  if (child.status !== 0 || Buffer.byteLength(child.stderr ?? '') !== 0 || !existsSync(replayOutput)) throw new Error('BR6A verify-replay child failed');
  if (!filesByteIdentical(planOutput, replayOutput)) throw new Error('BR6A_NONDETERMINISTIC_REPLAY');
}
