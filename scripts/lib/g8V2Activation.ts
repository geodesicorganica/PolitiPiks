...
function addActivationMetadata(data: Json, kind: ReturnType<typeof classifySourcePath>, sourcePath: string) {
  ...
  return { ...data, ...expected, canonicalActivation: { contract: G8_V2_ACTIVATION_CONTRACT, generation: CERTIFIED_G8_PRODUCT_SHADOW.generation, sourcePath } };
}

function selector(plan: Pick<G8V2ActivationPlan, 'target' | 'sourceCommit' | 'certifiedDigests' | 'expectedCounts' | 'authorizationReceipts'>, state: G8V2ActivationState) {
...
function assertActivationPlan(plan: G8V2ActivationPlan) {
  if (plan.schemaVersion !== 1 || plan.contract !== G8_V2_ACTIVATION_CONTRACT || plan.manifestPath !== G8_V2_ACTIVATION_MANIFEST_PATH) throw new Error('malformed g8.3a activation plan');
  if (plan.generation !== CERTIFIED_G8_PRODUCT_SHADOW.generation || plan.immutableLegacyGeneration !== 'canonical-2026-shadow-v1' || !/^[a-f0-9]{7,64}$/i.test(plan.sourceCommit)) throw new Error('invalid g8.3a activation identity');
...
export function buildG8V2ActivationPlan(shadowPlan: G8ProductShadowWritePlan, authorizationReceipts: G8V2AuthorizationReceipts, sourceCommit = shadowPlan.sourceCommit): G8V2ActivationPlan {
  ...
  const documents = shadowPlan.documents.map((source: ShadowDocument) => {
    ...
    return { path: activePath(source.path, namespace), data: addActivationMetadata(structuredClone(source.data), kind, source.path) };
