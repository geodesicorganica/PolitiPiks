...
import { buildResearchMetricsBaseline, validateResearchMetricsBaselineSnapshot, type ResearchMetricsBaselinePlan } from './researchMetricsBaseline.js';
...
export function buildFecFinanceBaselinePlan(snapshotValue: unknown, captureValue: unknown): FecFinanceBaselinePlan {
  ...
  const capture = validateFecFinanceCaptureSnapshot(captureValue); const evidenceDigest = digest({ baseline: baseline.evidenceDigest, finance: depth.evidenceDigest, documents }); const coverage = { ...baseline.coverage, metrics: { ...baseline.coverage.metrics, depth: { ...baseline.coverage.metrics.depth, comparativeFinance: { present: depth.audit.comparativePresent, unavailable: 470 - depth.audit.comparativePresent, not_applicable: 0 } } } };
  return { ...baseline, documents, inputDigest: digest({ baseline: baseline.inputDigest, financeCapture: capture.inputDigest }), evidenceDigest, planDigest: digest({ documents, coverage, finance: depth.audit, evidenceDigest }), coverage, financeCaptureDigest: capture.inputDigest, financeAudit: depth.audit, sourceSnapshotInputDigest: snapshot.inputDigest };
}
