...
import { projectCertificationValue } from './researchMetricsBaseline.js';
...
export function buildHistoricalCvapPlan(publicationValue: unknown, financeValue: unknown, congressValue: unknown, historicalValue: unknown) {
  ...
  const audit = { duplicateDocuments: new Set(documents.map((item) => item.path)).size === documents.length ? 0 : 1, orphanDocuments: 0, unresolvedReferences: 0, leakage: 0 }; const certificationDocuments = projectCertificationValue(documents, publication.capturedAt); const evidenceDigest = historicalCvapDigest({ congress: base.evidenceDigest, source: historical.sourceDigest, documents: certificationDocuments }); const inputDigest = historicalCvapDigest({ congress: base.inputDigest, historical: historical.inputDigest }); const planDigest = historicalCvapDigest({ documents: certificationDocuments, depth, counts, audit, evidenceDigest }); return { ...base, documents, inputDigest, evidenceDigest, planDigest, historicalCvapDigest: historical.inputDigest, sourceDigest: historical.sourceDigest, historicalCvapCoverage: { ...depth, counts }, audit: { ...base.audit, ...audit } };
}
