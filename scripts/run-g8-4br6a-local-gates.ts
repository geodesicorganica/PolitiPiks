...
const receiptPath = resolve(privateRoot, 'g8-4br6a-final-local-gate-receipt.json');
...
const gates: Gate[] = [
  { label: 'br6a-focused', executable: process.execPath, arguments: [tsxCli, 'scripts/lib/g8V2ConflictDisposition.test.ts'] },
...
function main() {
  expect(process.argv.length === 2, 'BR6A_GATE_RUNNER_ACCEPTS_NO_ARGUMENTS');
  expect(existsSync(npmCli), 'BR6A_LOCAL_NPM_CLI_MISSING');
  expect(!existsSync(receiptPath), 'BR6A_GATE_RECEIPT_COLLISION');
  ...
  expect(!emulatorKeys.includes('FIREBASE_SERVICE_ACCOUNT') && !emulatorKeys.includes('GOOGLE_APPLICATION_CREDENTIALS') && !emulatorKeys.includes('FIRESTORE_EMULATOR_HOST'), 'BR6A_EMULATOR_CREDENTIAL_OR_HOST_LEAK');
  expect(!emulatorKeys.includes('HTTP_PROXY') && !emulatorKeys.includes('HTTPS_PROXY') && !emulatorKeys.includes('ALL_PROXY'), 'BR6A_EMULATOR_PROXY_LEAK');
  expect(emulatorEnvironment.NO_PROXY === '127.0.0.1,localhost,::1' && emulatorEnvironment.no_proxy === '127.0.0.1,localhost,::1' && emulatorEnvironment.FIREBASE_CLI_DISABLE_UPDATE_CHECK === 'true', 'BR6A_EMULATOR_LOOPBACK_POLICY_MISMATCH');
  const before = snapshotIdentity();
  expect(before.bytes === expectedSnapshotBytes && before.sha256 === expectedSnapshotSha256, 'BR6A_GATE_SNAPSHOT_MISMATCH_BEFORE');
  const results: Json[] = [];
  for (const gate of gates) {
    ...
    if (afterGate.bytes !== before.bytes || afterGate.sha256 !== before.sha256) {
      writeExclusive({ contract: 'g8-4br6a-local-gate-receipt/v1', status: 'failed', failureClass: 'SNAPSHOT_DRIFT', results, snapshot: { before, afterGate }, safety: { productionOperations: 0 } });
      process.exitCode = 1; return;
    }
    if (child.status !== 0) {
      writeExclusive({ contract: 'g8-4br6a-local-gate-receipt/v1', status: 'failed', failureClass: 'NONZERO_GATE', failedGate: gate.label, results, snapshot: { before, after: afterGate }, safety: { productionOperations: 0 } });
      process.exitCode = 1; return;
    }
  }
  const after = snapshotIdentity();
  const receipt = {
    contract: 'g8-4br6a-local-gate-receipt/v1', status: 'completed', results,
...
try { main(); }
  ...
  const failure = { contract: 'g8-4br6a-local-gate-receipt/v1', status: 'failed', failureClass: error instanceof Error ? error.message : 'BR6A_UNKNOWN_GATE_FAILURE', safety: { productionOperations: 0 } };
